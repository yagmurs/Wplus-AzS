 @lab Word Wrap  Preview  Autosave  Theme    
12345678910111213141516171819202122232425262728293031323334353637383940414243
#WorkshopPLUS – Architecting Hybrid Cloud Solutions – Azure Stack #Introduction Microsoft Azure Stack is a hybrid cloud platform that lets you provide Azure services from your datacenter. You'll need an awareness of which services you can make available to your users. Azure Stack supports a subset of Azure services. The list of supported services will continue to evolve. ##Foundational servicesBy default, Azure Stack includes the following "foundational services" when you deploy Azure Stack:1. Compute1. Storage1. Networking1. Key Vault With these foundational services, you can offer Infrastructure-as-a-Service (IaaS) to your users with minimal configuration. ##Additional servicesCurrently, we support the following additional Platform-as-a-Service (PaaS) services:1. App Service1. SQL databases1. MySQL databases These services require additional configuration before you can make them available to your users. Throughout this lab you will be deploying Azure Stack Development kit and enable above services acting as Azure Stack Operator role. Your users want to use services. From their perspective, your main role is to make these services available to them. You must decide which services to offer, and make those services available by creating plans, offers, and quotas. In short you will learn how to deploy Azure Stack Development Kit, how to manage the infrastructure, and how to offer services. In addition you will learn tools needed to manage and monitor the infrastructure. Click **Next** to advance to [**Table of content**](#toc) window ===######toc#Table of Content>[**Lab 01**](#lab01)-   [Exercise 1](#lab01e1)-   [Exercise 2](#lab01e2)-   [Exercise 3](#lab01e3)-   [Exercise 4](#lab01e4)-   [Exercise 5](#lab01e5) >[**Lab 02**](#lab02)-   [Exercise 1](#lab02e1) >[**Lab 03**](#lab03)
...
Click Next to advance to Table of content window
...
Exercise 4
Exercise 5
Click Next to advance to LAB 01 window
...
Click Next to advance to LAB 01 - Exercise 1 window
...
Logon Azure Portal using following information;

Username:	@lab.UserEmail
Password:	<\Your Password>
If you don't have Microsoft Account, please Create a new Microsoft Account

Register Azure Pass

In the Azure Portal, click More services, and then Azure Active Directory
...
Tenant user now created, needs to be the Owner for the Azure subscription

Task 2: Delegate the account on subscription
In this task, you will:

Delegate the account on subscription to be able to manage subscription and register Azure Stack on following lab exercises
In the Azure Portal, click More services, and then Subscriptions
...
This will make sure that you have changed the temporary password

Above instructions have configured Azure Active Directory to start Azure Stack Development Kit (ASDK) installation

Now, we may start deploying Azure VM to host ASDK
...
Navigate to https://github.com/yagmurs/AzureStack-VM-PoC
...
Wait for the deployment to complete. This should take about 10 to 20 minutes depends on the region and load.

Result: After completing this exercise, you have deployed Azure VM to host Azure Stack using ARM template and created addtional Global Administrator account on Azure Active Directory

Click Next to advance to LAB 01 - Exercise 2 window
...
In the Azure Portal, click More services, and then Virtual Machines
...
Admin Password:	@lab.VirtualMachine(55267).Password
Select:	1
Unless you instructed by the trainer, always select 1 to install the latest ASDK version

Enter to continue
Above automated setup will continue as follows:
...
more...
During ASDK deployment you may lose connectivity to host due to network configuration changes.

Azure Stack host will also reboot during the deployment to join the host to AZURESTACK domain.

To follow setup progress after reboot, logon using AZURESTACK\AzureStackAdmin and password @lab.VirtualMachine(55267).Password

This process takes about 5-6 hours to complete

Result: After completing this exercise, you have successfully deployed Azure Stack Development Kit (ASDK)

Click Next to advance to LAB 01 - Exercise 3 window
...
Now Azure Stack Development Kit is installed
...
Password	: @lab.VirtualMachine(55267).Password
Logon Azure Stack Admin Portal using following information;

Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
On Main menu click +New > Offers + Plans and examine the Offers, Plans and Quotas

Do not set up any of them yet, we will be configuring later. Close Offers + Plans blade

On Main menu click +New > Compute. Notice that there are only two items yet, Availability Set and Storage Account

Do not set up any of them yet, we will be configuring later. Close Compute blade
...
Logon Azure Stack Tenant Portal using following information;

Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
Notice that background color has been changed to Blue from Grey, meaning you are logged on as a normal tenant, you do not have administrative rights. Most of the items are missing like Marketplace Management, Plans, Offers
...
Within the following exercise we will continue to setup management environment as a stepping stone to register Azure Stack to Azure Active Directory

Click Next to advance to LAB 01 - Exercise 4 window
...
Within the following exercise we will continue to setup management environment as a stepping stone to register Azure Stack to Azure Active Directory Reference Document

Task 2: Download Azure Stack tools and extract
AzureStack-Tools is a GitHub repository that hosts PowerShell modules for managing and deploying resources to Azure Stack
...
As an Azure Stack operator, you can configure your Azure Stack Development Kit's PowerShell environment. After you configure, you can use PowerShell to manage Azure Stack resources such as creating offers, plans, quotas, managing alerts, etc. by PowerShell
...
Now we have configured PowerShell, we can test the configuration by creating a resource group

PowerShell
New-AzureRMResourceGroup -Name "Test-Resource-Group" -Location Local
Click Next to advance to LAB 01 - Exercise 5 window
...
Registration is recommended because it enables you to test important Azure Stack functionality, like marketplace syndication and usage reporting. After you register Azure Stack, usage is reported to Azure commerce. You can see it under the subscription you used for registration. Azure Stack Development Kit users are not charged for any usage they report.

more...
CloudAdmin user account is introduced here has been created as an account on AZURESTACK Active Directory for internal use. Password of CloudAdmin will be as same as @lab.VirtualMachine(55267).Password as have been configured for other users, this is valid only for ASDK deployment.
...
The output will be similar to as follows: Lab Image

Reference Document

Click Next to advance to LAB 02 - Objectives and Summary window
...
Lab 02 - Marketplace
Objectives and Summary
Click Next to advance to LAB 02 - Exercise 1 window
...
Click Next to advance to LAB 03 - Objetives and Summary window
...
Objectives and Summary
Click Next to advance to LAB 03 - Exercise 1 window
...
Result: After completing this exercise, you have created the Active Directory accounts you will use in this lab

Click Next to advance to LAB 03 - Exercise 2 window
...
In this exercise, you will act as a cloud operator and create a plan consisting of the Subscription service and the corresponding offer. Next, you will create a subscription containing the new offer and designate the delegated provider as its subscriber:
...
Wait for the deployment to complete. This should take just a few seconds.
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click New
...
Result: After completing this exercise, you have designated the delegated provider

Click Next to advance to LAB 03 - Exercise 3 window
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click Offers
...
Result: After completing this exercise, you have created and delegated the offer to a delegated provider.

Click Next to advance to LAB 03 - Exercise 4 window
...
Create an offer for a tenant (as a delegated provider) based on the cloud operator offer for the delegated provider This will allow tenants to create a subscription based on the offer from the delegated provider
Logon Azure Stack host VM with RDP client using following information;
...
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Tenant Portal, click Offers
...
Username	: delegatedprovider@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Tenant Portal, click More services
...
Sign out from the Azure Stack Tenant Portal
Tenants need to subscribe to the offer from the delegated provider portal URL

Result: After completing this exercise, you have created a delegated provider offer

Click Next to advance to LAB 03 - Exercise 5 window
...
Make sure NOT to use the Azure tenant portal URL in this case

In the Azure Stack Tenant Portal, click Get a Subscription on the dashboard
...
Create a resource in the new subscription (as a tenant user-tenantuser)
Back in the Azure Stack Tenant Portal, click More services
...
Result: After completing this exercise, you have subscribed to a delegated provider’s offer, creating this way a new subscription and provisioned a storage account in the new subscription.

Click Next to advance to LAB 04 - Objectives and Summary window
...
Objectives and Summary
Click Next to advance to LAB 04 - Exercise 1 window
...
This template is being downloaded from Azure Stack Quickstart templates gallery on github.com

Template may be called as in following format. Inside the URI which is called there is a redirection to the address below displayed
...
more...
Click Next to advance to LAB 04 - Exercise 2 window
...
Navigate https://github.com/Azure/AzureStack-QuickStart-Templates
...
Click Next to advance to LAB 05 - Objectives and Summary window
...
Objectives and Summary
Click Next to advance to LAB 05 - Exercise 1 window
...
Result: After completing this exercise, you have enabled Virtual Machine scale sets on your Azure Stack deployment

Click Next to advance to LAB 05 - Exercise 2 window
...
Username	: tenantuser@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Tenant Portal, click New
...
In the Azure Stack Tenant Portal, click Resource Groups
...
Username: \localadm
Password: @lab.VirtualMachine(55267).Password
Result: After completing this exercise, you have deployed Virtual Machine scale sets using tenant subscription

Click Next to advance to LAB 05 - Exercise 3 window
...
Password	: @lab.VirtualMachine(55267).Password
Open PowerShell ISE Run as an Administrator. Copy following script to Powershell ISE and replace < > fields between brackets with the values represent your environment.
The following Powershell script will set up tenant PowerShell environment and logon using Powershell to manage Azure Stack cloud resources
...
Password	: @lab.VirtualMachine(55267).Password
Open PowerShell ISE Run as an Administrator. Copy following script to Powershell ISE and replace < > fields between brackets with the values represent your environment.
The following Powershell script scales Virtual Machine scale set in rg01 resource group Intances to 2
...
Wait for the deployment to complete. This should take about 15 minutes
...
Because of the default load balancer rules, port 50001 on the Load Balancer will be mapped to the second VM in the scale set on port 3389.
...
Username: \localadm
Password: @lab.VirtualMachine(55267).Password
Result: After completing this exercise, you have connected to your tenant subscription using PowersShell, manually scaled up Virtual Machine scale set, and validated the connectivity to all intances

Click Next to advance to LAB 06 - Objectives and Summary window
...
Objectives and Summary
Click Next to advance to LAB 06 - Exercise 1 window
...
In this exercise, you will act as cloud operator to configure retention policy, recover storage account, and act as a tenant user to create, delete storage account, and confirm recovery of the storage account:
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click Dashboard
...
Wait for processing to complete. This should take less than a minute
...
Result: After completing this exercise, you have configured retention policy, deletes storage account, and recovered storage account

Click Next to advance to LAB 06 - Exercise 2 window
...
Reclaim space process will force garbage collection of all deleted storage accounts, regardless of the retention period setting

Result: After completing this exercise, you have manually triggered garbage collection by clicking Reclaim space button

Click Next to advance to LAB 07 - Objectives and Summary window
...
Lab 07 -
Objectives and Summary
Click Next to advance to LAB 07 - Exercise 1 window
...
Click Next to advance to LAB 07 - Exercise 2 window
...
Click Next to advance to LAB 08 - Objectives and Summary window
...
Objectives and Summary
Click Next to advance to LAB 08 - Exercise 1 window
...
In the Azure Stack Admin Portal, click Dashboard
In the Dashboard click local in Region management list
The list of active alerts for each Azure Stack region is available directly from the Region management blade. The first tile in the default configuration is the Alerts tile, which displays a summary of the critical and warning alerts for the region.
...
In this task, you will: view the total number of public IP addresses that have been consumed in the region
...
In this task, you will: view list of all storage accounts
...
Click Next to advance to LAB 08 - Exercise 2 window
...
PowerShell
Test-AzureStack
If any tests report fail, run: Get-AzureStackLog -FilterByRole "rolename" -OutputPath "Log output path". The cmdlet gathers the logs from Test-AzureStack 5px64167.jpg

Result: After completing this exercise,

Click Next to advance to LAB 09 - Objectives and Summary window
...
Objectives and Summary
Click Next to advance to LAB 09 - Exercise 1 window
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click More Services
...
Result: After completing this exercise, you have installed a SQL resource providers and published marketplace items to Azure Stack.

Click Next to advance to LAB 09 - Exercise 2 window
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click ServiceName (SQL)
...
Login name: localsa
Password: @lab.VirtualMachine(55267).Password
On the Create Virtual Machine blade, STEP 5 - Summary specify the following information: > - Verify and click OK
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click Plans
...
Click OK twice
Result: After completing this exercise, you have deployed SQL server to enable SQL DB PaaS services on Azure Stack

Click Next to advance to LAB 09 - Exercise 3 window
...
Result: After completing this exercise, you have deployed a SQL database to demostrate Azure Stack SQL DB PaaS and validated the database deployment

Click Next to advance to LAB 10 window
...
Lab 10 -
Objectives and Summary
Click Next to advance to LAB 10 - Exercise 1 window
...
Task 1: Validate File Server installed for App Service on Lab 04
In this task, you will:
...
Username	: @lab.UserFirstName@<\tenant_name>.onmicrosoft.com
Password	: @lab.VirtualMachine(55267).Password
In the Azure Stack Admin Portal, click Virtual Machines
...
The name is supposed to be appservicefileshare.local.cloudapp.azurestack.external

This will be used on App Service Registration later within this lab.
...
Wait for deployment to finish, this will take approximately 20 minutes

Result: After completing this exercise, you have

Click Next to advance to LAB 10 - Exercise 2 window
...
Result: After completing this exercise, you have downloaded and extracted the App Service installer and helper scripts before you start installing App Service installation on Azure Stack Development Kit

Click Next to advance to LAB 10 - Exercise 3 window
...
Click Next to advance to LAB 10 - Exercise 4 window
...
Click Next to advance to LAB 10 - Exercise 5 window
...
Click Next to advance to LAB 11 - Objectives and Summary window
